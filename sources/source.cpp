...
std::string MakeJson(const json& data) {
  std::stringstream ss;
  if (data.is_null())
    ss << "No suggestions\n";
  else
...
void handle_request(http::request<Body, http::basic_fields<Allocator>>&& req,
                    ...
                    const std::shared_ptr<SuggestionsCollection>& collection) {
  ...
  mutex->unlock();
  http::string_body::value_type body = MakeJson(result);
...
void do_session(net::ip::tcp::socket& socket,
                [[maybe_unused]] const std::shared_ptr<SuggestionsCollection>& collection,
                [[maybe_unused]] const std::shared_ptr<std::timed_mutex>& mutex) {
  bool close = false;
  beast::error_code ec;
...
void SuggestionUpdater(
    const std::shared_ptr<JsonStorage>& storage,
    const std::shared_ptr<SuggestionsCollection>& suggestions,
    const std::shared_ptr<std::timed_mutex>& mutex) {
  for (;;) {
    mutex->lock();
    storage->Load();
    suggestions->Update(storage->GetStorage());
    mutex->unlock();
...
int Run_server(int argc, char* argv[]) {
  ...
  std::shared_ptr<JsonStorage> storage = std::make_shared<JsonStorage>(
      "/Users/yury/Desktop/lab-07-http-server/cmake-build-debug/"
      "suggestions.json ");
  std::shared_ptr<SuggestionsCollection> suggestions =
      std::make_shared<SuggestionsCollection>();
  try {
    if (argc != 3) {
    ...
    std::thread{SuggestionUpdater, storage, suggestions, mutex}.detach();\
